...
@section('content')
    ...
    @if ($totals->isNotEmpty())
        <div class="card p-4 mb-6">
            <h2 class="text-xs uppercase tracking-wider text-ink-500 mb-3">Totales por proyecto · semana</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-7 gap-3">
        @foreach ($days as $d)
            ...
            <a href="{{ route('timeline.day', ['date' => $d['date']->format('Y-m-d')]) }}"
               class="card p-3 hover:border-ink-700 transition block min-h-[180px] {{ $isToday ? 'ring-1 ring-emerald-400/50' : '' }}">
                ...
                @if ($d['by_project']->isEmpty())
                    <p class="text-xs text-ink-600 mt-4 text-center">—</p>
                @else
                    <ul class="space-y-1">
                            ...
                            <li class="flex items-center justify-between gap-1 text-xs">
                                @if ($row['project'])
                                    <span class="inline-flex items-center gap-1 truncate"
                                          style="color: {{ $row['project']->color ?? '#9ca3af' }}">
                                        <span class="inline-block w-1.5 h-1.5 rounded-full"
                                              style="background: {{ $row['project']->color ?? '#9ca3af' }}"></span>
                                        {{ $row['project']->code }}
                                    </span>
                                @else
                                    <span class="text-ink-500 truncate">—</span>
                                @endif
                                <span class="font-mono text-ink-400">
                                    {{ intdiv($row['minutes'], 60) }}h{{ str_pad($row['minutes'] % 60, 2, '0', STR_PAD_LEFT) }}
                                </span>
                            </li>
                        @endforeach
                        @if ($d['by_project']->count() > 4)
                            <li class="text-[10px] text-ink-600">+ {{ $d['by_project']->count() - 4 }} más</li>
                        @endif
                    </ul>
                @endif
            </a>
