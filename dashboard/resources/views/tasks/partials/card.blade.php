...
<div class="task-card card p-2.5 cursor-grab active:cursor-grabbing relative
    ...
    @if ($hasChips)
        <div class="flex flex-wrap items-center gap-1 mt-2">
            ...
            @endif
            @if ($logged > 0)
                <span class="chip" title="{{ __('tasks.tracked_time') }}"><x-icon name="clock" class="w-3 h-3" />{{ $logged >= 60 ? intdiv($logged, 60) . 'h ' . ($logged % 60) . 'm' : $logged . 'm' }}</span>
            @endif
            @if ($checkboxAll > 0)
                <span class="chip {{ $checkboxDone === $checkboxAll ? 'text-emerald-600 dark:text-emerald-400' : '' }}"
                      title="{{ __('tasks.subtasks_label') }}" data-card-subtasks-badge><x-icon name="check" class="w-3 h-3" />{{ $checkboxDone }}/{{ $checkboxAll }}</span>
            @endif
            @if ($comments->isNotEmpty())
                <span class="chip" title="{{ __('tasks.comments_label') }}" data-card-comments-badge><x-icon name="chat" class="w-3 h-3" />{{ $comments->count() }}</span>
            @endif
        </div>
    @endif
